...
review sheet. Only suggestions at or above
:data:`config.AUTO_LINK_MIN_SCORE` are applied without a human look.
"""
...
def _without_colour(base: str, colour: str) -> str:
    ...
    Colour is scored separately, so leaving it in the base would penalise a
    correct pair twice.
    """
...
def score_pair(pos_row: PosRow, sellup_row: SellUpRow) -> Suggestion | None:
    ...
    # land on. This keeps laptops, chargers and styluses out entirely.
...
class SuggestionIndex:
    ...
    comparisons. Bucketing by (manufacturer, storage) cuts that to a few dozen
    candidates per POS row.
    """
