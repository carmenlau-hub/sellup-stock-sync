...
_MASTERLIST_HEADERS = {
    ...
    "ML Available Qty", "Target Stock", "Masterlist Stock Type ID",
    "Category", "Brand", "Model", "Color", "Available Qty",
    "Corrected Masterlist ID",
}
_INPUT_HEADERS = set(config.NEW_SKUS_INPUT_COLUMNS)

_COLUMN_WIDTHS: dict[str, float] = {
    "#": 5, "SellUp Sheet": 14, "SellUp SKU ID": 17, "SellUp Model": 30,
    "Storage": 10, "Connectivity": 20, "SellUp Colour": 20, "Condition": 20,
    "LOCKED Masterlist ID(s)": 24, "ML Category": 11, "ML Model(s)|Color": 40,
    "ML Available Qty": 16, "Target Stock": 12, "# SKUs": 8, "How linked": 14,
    "Masterlist Stock Type ID": 22, "Brand": 14, "Model": 34, "Color": 20,
    "Available Qty": 13, "Suggested SellUp SKU ID": 22,
...
def _write_rows(worksheet, rows: Iterable[Sequence[object]], header_count: int) -> int:
    ...
    for offset, row in enumerate(rows, start=2):
        for idx, value in enumerate(row, start=1):
            worksheet.cell(offset, idx, value).font = _BODY_FONT
        written += 1
    if written:
...
def _add_decision_dropdown(worksheet, headers: Sequence[str], column: str) -> None:
...
def load_registry(source: str | IO[bytes]) -> Registry:
    ...
    try:
        ...
        # Link History first -- the complete map, including links whose POS
        # row is out of stock today.
        ...
        if id_col and dec_col:
            for row in range(2, worksheet.max_row + 1):
                ...
                # Filling in a SKU counts as a link even when the decision
                # column was left blank -- the common shorthand in Excel.
                ...
                if decision not in config.TERMINAL_DECISIONS:
                    continue
                # "Do Not Link" must not carry a SKU through.
...
def _summary_sheet(workbook, result: PipelineResult, generated: str) -> None:
    ...
    m = result.metrics()
    rows = [
        ("Locked Matches (stock synced)", m["locked_updated"]),
        ("  linked by reviewer", m["reviewer_linked"]),
        ("  linked automatically", m["auto_linked"]),
        ("  crosswalk links overridden", m["displaced"]),
        ("Still to review in this file", m["requiring_review"]),
        ("Not Selling in SellUp", m["not_selling"]),
        ("Not on SellUp Yet", m["not_yet"]),
        ("Do Not Link (suppressed)", m["suppressed"]),
        ("Match Review (listings with no source)", m["orphan_listings"]),
        ("Validation errors & warnings", m["validation_errors"]),
        ("Quantity cells written", m["cells_to_write"]),
        ("Listings delisted (set to 0)", m["delisted"]),
        ("Total units synced", m["units_synced"]),
    ]
    ...
    for line in (
        ...
        "A reviewer link always beats the crosswalk. Use 'Do Not Link' to stop",
        "a row being matched to anything at all.",
    ):
        cell = worksheet.cell(note_row, 2, line)
        cell.font = Font(size=9, italic=not line.endswith(":"), bold=line.endswith(":"))
        note_row += 1

    worksheet.cell(note_row + 1, 2, f"Generated {generated}").font = Font(
        size=9, italic=True
    )
...
def _locked_rows(locked: list[LockedMatch]) -> list[list[object]]:
    return [
        [
            idx, m.sellup.sheet, m.sellup.sku_id, m.sellup.model,
            m.sellup.storage_label, m.sellup.connectivity_label, m.sellup.colour,
            m.slot, m.masterlist_ids, m.masterlist_categories, m.masterlist_labels,
            m.available_quantities, m.target_stock, len(m.pos_rows), m.origin,
        ]
...
def _suggestion_cells(item: NewMasterlistSku) -> list[object]:
    """The seven suggestion columns for one review row."""
...
def _new_sku_rows(new_skus: list[NewMasterlistSku]) -> list[list[object]]:
    return [
        [
            idx, item.pos.stock_type_id, item.pos.category, item.pos.brand,
            item.pos.model, item.pos.colour, item.pos.available_qty, item.pos.slot,
            *_suggestion_cells(item),
            item.linked_sku_id, item.decision, item.notes,
        ]
...
def _unsold_rows(pos_rows) -> list[list[object]]:
    return [
        [idx, r.stock_type_id, r.category, r.brand, r.model, r.colour, r.available_qty]
...
def build_registry_workbook(result: PipelineResult, generated: str) -> bytes:
    ...
    sheets: list[tuple[str, Sequence[str], list[list[object]]]] = [
        ...
        (config.SHEET_MATCH_REVIEW, config.MATCH_REVIEW_HEADERS,
         _match_review_rows(result.match_review)),
        (config.SHEET_NOT_SELLING, config.UNSOLD_HEADERS,
         _unsold_rows(result.not_selling)),
        (config.SHEET_NOT_YET, config.UNSOLD_HEADERS, _unsold_rows(result.not_yet)),
        (config.SHEET_LINK_HISTORY, config.LINK_HISTORY_HEADERS,
    ...
    _add_decision_dropdown(
        workbook[config.SHEET_NEW_SKUS], config.NEW_SKUS_HEADERS, "Reviewer Decision"
    )
    _add_decision_dropdown(
        workbook[config.SHEET_MATCH_REVIEW], config.MATCH_REVIEW_HEADERS,
        "Reviewer Decision",
    )
